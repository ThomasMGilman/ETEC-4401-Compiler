...
public class Assembler
{
    ...
    static Dictionary<string, StructVarType> structures;
    ...
    private void programNodeCode(TreeNode n)
    {
        ...
        emit("global main");
        int dontCare;
        structdecllistNodeCode(n.Children[0]);
        vardecllistNodeCode(n.Children[1], 0, out dontCare);
        funcdecllistNodeCode(n.Children[2], true);
        funcdecllistNodeCode(n.Children[2], false);
        emit("main:");
    ...
    /// </summary>
    /// <param name="n"></param>
    private void structdecllistNodeCode(TreeNode n)
    {
    ...
    /// </summary>
    /// <param name="n"></param>
    private void structdeclNodeCode(TreeNode n)
    {
        string name = n.Children[1].Token.Lexeme;
        if (structures.ContainsKey(name))
            throw new Exception("ERROR ICE!!! Duplicate decleration of struct: " + name);

        var tmp = new List<Tuple<string, VarType>>();
    ...
    /// </summary>
    /// <param name="n"></param>
    private void structmemberdecllistNodeCode(TreeNode n, List<Tuple<string, VarType>> members)
    {
        string name; VarType type;
        while(true)
        {
            structmemberNodeCode(n.Children[0], out name, out type);
            members.Add(new Tuple<string, VarType>(name, type));
    ...
    /// </summary>
    /// <param name="n"></param>
    private void structmemberNodeCode(TreeNode n, out string name, out VarType type)
    {
    ...
    private void funccallNodeCode(TreeNode n, out VarType type)
    {
        ...
        else if(n.Children.Count == 4)
        {
            ...
            foreach (var x in actualTypes)
                totalSize += x.sizeOfThisVariable;
            if(totalSize > 0)
                emit("add rsp, {0}", totalSize);
        ...
        switch (sym)
        {
            ...
            case "STRUCT":
                ...
                if (!structures.ContainsKey(sname))
                    throw new Exception("ERROR ICE!!! Struct does not exsist yet within scope!!! Struct wanted: "+ sname);
                type = structures[sname];
                break;
            default:
    ...
    private void assignNodeCode(TreeNode n)
    {
        ...
        else
        {
            ...
            if (childrenCount == 3)         //Scalar assignment
            {
                ...
                var atyp = rhsType as ArrayVarType;
                ...
                {
                    if (vinfo.VType != rhsType)
                        throw new Exception("Error!! Type Mistmatch: " + vinfo.VType + "!=" + rhsType);
            ...
            else                            //Array assignment
            {
                ...
                if (typ.baseType != rhsType)
                    throw new Exception("Error!! Type mismatch!! " + typ.baseType + "!=" + rhsType);
    ...
    private void factorNodeCode(TreeNode n, out VarType type)
    {
        ...
        switch (child.Symbol)
        {
            ...
            case "func-call":
                ...
                if (type.sizeOfThisVariable != 8)
                    throw new Exception("ERROR!!! cant return non-number/string right now!!");
                emit("push rax");
                break;
            case "array-access":
                ...
                putArrayAddressInRcx(vi, n.Children[0]);
            ...
            case "struct-member-access":
                ...
                break;
            default:
                throw new Exception("ICE!!! Expected NUM, LP, STRING-CONSTANT, ID, arrayAccess, or structMemberAccess. Recieved:" + child.Symbol);
        }
    }
    ...
    /// </summary>
    /// <param name="n"></param>
    private void structMemberAccessNodeCode(TreeNode n, out VarType type)
    {
        var ids = new List<string>(); TreeNode x = n;
        while (true)
        {
            ids.Add(x.Children[0].Token.Lexeme);
            if (x.Children[2].Symbol == "ID")        //ID DOT ID
            {
                ids.Add(x.Children[2].Token.Lexeme);
                break;
            }
        ...
        for (int i = 1; i < ids.Count; ++i) //go through the structs contents
        {
            ...
            if (svtype == null)
                throw new Exception("ERROR!!! Item to the left of . is not a struct!!! Type: " + svtype.typeString);
            string name = ids[i];
            var j = svtype.Members.FindIndex(y => name == y.Item1);
            if (j == -1)
                throw new Exception("ERROR!!! Field dose not exist!! Name: " + name);

            type = svtype.Members[j].Item2;
            var offset = 0;
            for (int k = 0; k < j; ++k)
                offset += svtype.Members[k].Item2.sizeOfThisVariable;
            emit("add rax, {0}", offset);
        }
    }
    ...
    private void copyStackToVar(string destination, int sizeInBytes)
    {
        if (destination != "rax")
            emit("lea rax, [{0}]", destination);
        if (sizeInBytes == 8)
            emit("pop qword [rax]");
        else
        {
            if (sizeInBytes % 8 != 0 || sizeInBytes == 0)
                throw new Exception("ERROR ICE!!! Size of data to copy is not a multiple of 8 or is zero!! Size: " + sizeInBytes);
            int sizeInQuads = sizeInBytes / 8;
            var startLoop = label();
            emit("mov rcx, {0}", sizeInQuads);
            emit("{0}:", startLoop);
            emit("pop qword [rax]");
            emit("add rax, 8");
            emit("sub rcx, 1");
    ...
    private void copyVarToStack(string destination, int sizeInBytes)
    {
        if (destination != "rax")
            emit("lea rax, [{0}]", destination);
        if (sizeInBytes == 8)
