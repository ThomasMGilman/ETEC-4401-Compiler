...
class VarType
{
    ...
    public static readonly VarType STRUCT   = new VarType("$struct");
...
class StructVarType : VarType
{
    public readonly string name;
    public readonly List<Tuple<string, VarType>> Members;
    public StructVarType(string name, List<Tuple<string, VarType>> Members) : base("$struct" + name)
    {
        this.name = name; this.Members = Members; size = 0;
        foreach (var m in Members)
            size += m.Item2.sizeOfThisVariable;
    }
    public static bool operator == (StructVarType v1, StructVarType v2)
    {
        if (object.ReferenceEquals(v1, null))
            return object.ReferenceEquals(v2, null);
        return v1.Equals(v2);
    ...
    public override bool Equals(object obj)
    {
        StructVarType v2 = (obj as StructVarType);
        if (v2 == null)                             return false;
        if (this.name != v2.name)                   return false;
        if (this.Members.Count != v2.Members.Count) return false;
        foreach(Tuple<string, VarType> mem in this.Members)
        {
            if (!v2.Members.Contains(mem)) return false;
        }
        return true;
    }
    public override int GetHashCode()
    {
        ...
        foreach (Tuple<string, VarType> mem in this.Members)
        {
            hash += mem.Item1.GetHashCode(); hash += mem.Item2.GetHashCode();
        }
        return hash;
    }
}
