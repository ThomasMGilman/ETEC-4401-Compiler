        ...
        while(inputIndex < tokens.Count)
        {
            ...
            if(inputStack.Count > 0)
            {
                if (productionDict.ContainsKey(inputStack.Peek().Symbol)) //top symbol is nonterminal
                {
                    ...
                    {
                        throw new Exception("Error: " + e.Message + " CurNode: " + curNode.Symbol + " input: " + t.Symbol + " lexeme: " + t.Lexeme);
                    }
                ...
                else if (inputStack.Peek().Symbol == t.Lexeme) //same symbol, pop
                {
                    ...
                    throw new Exception("Error: Symbol '" + t.Symbol + "' does not match top symbol '" + inputStack.Peek().Symbol + "'!!!");
            }
            else //start from start symbol
            {
                if (!LLTable[p.lhs].ContainsKey(t.Lexeme))
                    throw new Exception("Error: Token '" + t.Lexeme + " : " + t.Symbol + "' is not in LLTable under:'" + p.lhs + "'!!");
                else
                {
